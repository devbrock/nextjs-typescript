...
export default function Home({ characters }: CharacterProps) {
	...
	async function handleClick(character: { name: string; url: string }) {
		...
		let id = character.url
			.substring(0, character.url.length - 1)
			.split('/')
			.pop();
				...
				<div className="py-8">
					<ul className="flex space-x-4 justify-center">
						{characters.map((character) => (
							<li
								className="px-2 py-4 cursor-pointer bg-red-500 hover:bg-red-600 text-white font-semibold  rounded shadow"
								key={character.name}
					...
					<div className="py-8 grid grid-cols-2">
						...
						<div>
							...
							{selectedCharacter.vehicles.length > 0 && (
								<>
									<p className="font-mono font-semibold text-xl pt-4">
										Vehicles
									</p>
